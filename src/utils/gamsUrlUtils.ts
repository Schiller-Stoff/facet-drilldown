...
const detectInternalHref = (windowLocation: Location, hrefValue: string): boolean => {
  ...
  if (hrefValue === "#")
    throw new TypeError(`The href value of clicked element is invalid. It must not be just "${hrefValue}".`);
  if (hrefValue.length === 0 || !hrefValue)
    throw new TypeError(`The href value can't be an empty string or validate to false. Given value: ${hrefValue}`);
